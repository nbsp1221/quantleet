...
`quantleet` is a long-lived, agent-first quant system.
...
- one installable engine package under `src/quantleet`
...
`src/quantleet` contains the installable engine and library package.
...
`src/quantleet/cli` remains part of the packaged engine because CLI is a
