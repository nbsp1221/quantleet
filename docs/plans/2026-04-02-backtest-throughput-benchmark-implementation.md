...
measure `quantleet` and selected comparator libraries, and turn the findings
...
and keep repository-local artifacts focused on the `quantleet` baseline and the
final report.

**Tech Stack:** Python 3.13, `uv`, `quantleet`, `/tmp` comparator environments,
...
2. Add a repository-local `quantleet` benchmark
...
- produce a result table with first-run runtime, steady-state median runtime,
  and semantic notes
- recommend a concrete `quantleet` runtime target for the RSI optimization slice

## Verification

- `uv run poe verify`
- execute the repository-local `quantleet` benchmark script
