...
- Goal:
  - implement Stage 4 of the approved migration baseline by making
    `quantleet.integrations` the real owner of the current CCXT-backed venue
    protocol code
...
- In-repo scope:
  - create the first real `quantleet.integrations.venues` owner package for
    the current CCXT-backed market-data exchange logic
  - move the current CCXT-backed exchange implementation out of
    `quantleet.data.adapters.exchange_backend` into the new canonical owner
  - keep `quantleet.exchange` and
    `quantleet.data.adapters.exchange_backend` as compatibility shims only
...
- Success criteria:
  ...
  - `quantleet.data.adapters.exchange_backend` and `quantleet.exchange` are
...
- Notes:
  ...
  - read-only research split used bounded handoff contracts with disjoint
    scopes:
    - `Nash`: canonical owner placement under `quantleet.integrations`
  ...
  - final Stage 4 owned file group became:
    - `src/quantleet/integrations/venues/__init__.py`
    - `src/quantleet/integrations/venues/ccxt/__init__.py`
...
- read-only research split converged on the same Stage 4 shape:
  - the canonical owner should live under
    `quantleet.integrations.venues.ccxt.market_data`
  - `quantleet.data.adapters.exchange_backend` and `quantleet.exchange`
...
- the implementation now matches that Stage 4 ownership boundary:
  - CCXT-backed venue protocol code lives in
    `src/quantleet/integrations/venues/ccxt/market_data.py`
  - `src/quantleet/data/adapters/exchange_backend.py` is a shim that
    re-exports the canonical owner
  - `src/quantleet/exchange.py` is a shim that re-exports the canonical owner
  - `src/quantleet/data/adapters/ccxt_source.py` now imports its owner-facing
    CCXT helpers from the canonical integrations path
- the first targeted run exposed one real implementation issue:
  - importing through `quantleet.integrations.venues.ccxt.__init__` made the
  ...
  - this was corrected by pointing runtime imports and test patch points
    directly at `quantleet.integrations.venues.ccxt.market_data`
...
- Verification evidence:
  ...
  - read-only research split:
    - `Nash`: recommended `quantleet.integrations.venues.ccxt.market_data` as
