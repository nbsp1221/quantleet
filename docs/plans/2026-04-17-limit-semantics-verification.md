...
- In-repo scope:
  ...
  - repo-local inline execution against the current `quantleet` code
- Out-of-repo scope:
...
- Success criteria:
  - reproduce the original favorable-limit scenario on the current `quantleet` code
...
- Checks the evaluator will use:
  - fresh reproduction outputs from current `quantleet`
...
- Planned slice order:
  ...
  - run `quantleet` and at least one external conservative comparator on that same scenario
...
- Findings:
  ...
  - Fresh current-worktree `quantleet` output on the exact three-bar repro now fills the resting sell limit at `114.0`, not `115.0`.
  - Fresh external comparator output from `backtrader` on the same three-bar repro also fills the resting sell limit at `114.0`.
  - When costs are zeroed to isolate limit semantics, `quantleet` and `backtrader` match exactly on both fills and ending equity for the repro scenario.
  - A cost-bearing rerun still shows the same corrected sell-limit fill at `114.0` in `quantleet`; the remaining final-equity difference in that run comes from market-order slippage configuration, not from the limit-order bug under investigation.
- Verification evidence:
  - Fresh current `quantleet` repro with production-like costs:
  ...
  - Fresh zero-cost semantics-isolation rerun:
    - current `quantleet` output: `fills [('buy', 110.0, 120, 0.0), ('sell', 114.0, 180, 0.0)]`, `summary 1004.0 0.0 4.0`
